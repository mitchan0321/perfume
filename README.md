...
関数の中では、仮引数シンボルにローカル変数が割り当てられて現れる。
関数の中では、実引数の並び順にそれぞれの値が仮引数シンボルのローカル変数名として割り当てられる。
...
また、ブロック中で発生するExceptionを検知するために、tryコマンドが用意されている。これは、try ブロック catch: 例外ブロック fin: 後処理ブロック
